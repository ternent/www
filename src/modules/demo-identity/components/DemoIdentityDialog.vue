...
} from "@/modules/demo-identity/runtime";
...
const props = withDefaults(
  defineProps<{
    routeMode?: "create" | "recover" | "login";
  }>(),
  {
    routeMode: "create",
  },
...
const appApi = useDemoIdentityApi();
...
async function applyRouteMode(nextMode: "create" | "recover" | "login"): Promise<void> {
  if (nextMode === "recover") {
    startRecoverMode();
    return;
  }
...
onMounted(async () => {
  refreshSummary();
  dialogOpen.value = !isUnlocked.value && !hasPendingDevSessionResume();
  await applyRouteMode(props.routeMode);
});
</script>
